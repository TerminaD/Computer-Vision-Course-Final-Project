...
def render_rays(rays: torch.Tensor,
    ...
    xyzs_coarse_deformed = torch.clamp(xyzs_coarse + deformation_coarse, -1, 1)
    ...
    if with_depth:
        depths = torch.sum(weights_coarse * depths_coarse, dim=1)
    ...
    xyzs_all_deformed = torch.clamp(deformation_all + xyzs_all, -1, 1)
    ...
    if with_depth:
        return pixel_rgb_coarse, pixel_rgb_all, depths
    else:
