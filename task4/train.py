...
from utils.loss import NeRFMSEDepthLoss
...
def train() -> None:
    ...
    nerf_criterion = NeRFMSEDepthLoss()
    ...
    for e in range(args.epoch):
        ...
        for sample in tqdm(trainloader, desc="Training", leave=False):
            ...
            gt_depths = sample['depths'].to(device)
            
            optimizer.zero_grad()
            
            pred_rgbs_coarse, pred_rgbs_fine, pred_depths = render_rays(rays,
                                                           times,
